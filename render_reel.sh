...
echo "🎨 Starting FFmpeg Render: $TITLE"

# Prepare Assets
cp ./assets/spotify.png "$WORKDIR/spotify.png" || echo "Logo missing"
[ -f "$FONT" ] || curl -sL -o "$FONT" "https://github.com/googlefonts/BebasNeue/raw/main/fonts/ttf/BebasNeue-Regular.ttf"

# Trim 20s from the drop
python3 -c "
...
start = max(0, dur - 30)
subprocess.run(['ffmpeg', '-y', '-i', '$WORKDIR/audio.mp3', '-ss', str(start), '-t', '20', '-c:a', 'libmp3lame', '-b:a', '192k', '$WORKDIR/trimmed.mp3'])

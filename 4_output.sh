...
# Safe math (no bc dependency issues)
VIDEO_FADE_ST=$(awk "BEGIN {print $DURATION - 2}")
echo "Fade starts at: $VIDEO_FADE_ST"
...
[3:v]fps=25,scale=1920:1080,format=yuva420p,colorchannelmixer=aa=0.15[scr];
...
[1:a]afade=t=in:st=0:d=1.5,afade=t=out:st=$VIDEO_FADE_ST:d=2[a]
" \
-map "[v]" -map "[a]" \
...
# --- VERIFY OUTPUT ---
if [ ! -f "$FINAL_OUT" ]; then
  echo "❌ Render failed: output file missing"
  exit 1
fi

echo "✅ Success! Video created: $FINAL_OUT"
